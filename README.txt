...
The function "save_model" in script.lua saves models for each test. The tests done are defined in the
list "Tests_Todo". Each test trains the model with a particular combination of priors but the best one used now is
the one with all the priors.
Once you have saved a trained model, load_model.lua loads it using the variable "name"
